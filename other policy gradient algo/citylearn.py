...
class CityLearn(gym.Env):  
        ...
        for a_bld, building in zip(actions,self.buildings):
            ...
            for a in a_bld:
                ...
                for _ in range(self.time_resolution):                
                    ...
                    #print('demand',building_electric_demand)
            ...
            #print('soc',building.cooling_storage.soc)
            #self.state.append(np.array([s1, s2, s3]))
            hour_binary=np.zeros(26)
            hour_binary[int(s1)-1]=1
            hour_binary[24]=s2
            hour_binary[25]=s3
            self.state.append(hour_binary)
            #self.state.append(np.array([s1,s2,s3]))
    ...
    def reset(self):
        ...
        for building in self.buildings:
            ...
            #self.state.append(np.array([s1], dtype=np.float32))
            building.reset()
        return self._get_ob()
